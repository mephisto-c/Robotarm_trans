...
#if USMART_USE_HELP
			xprintf("------------------------USMART V2.8------------------------ \r\n");
			xprintf("    USMART是由ALIENTEK开发的一个灵巧的串口调试互交组件,通过 \r\n");
			xprintf("它,你可以通过串口助手调用程序里面的任何函数,并执行.因此,你可\r\n");
			xprintf("以随意更改函数的输入参数(支持数字(10/16进制)、字符串、函数入\r\n");
			xprintf("口地址等作为参数),单个函数最多支持10个输入参数,并支持函数返 \r\n");
			xprintf("回值显示.新增参数显示进制设置功能,新增进制转换功能.\r\n");
			xprintf("技术支持:www.openedv.com\r\n");
			xprintf("USMART有6个系统命令:\r\n");
			xprintf("?:    获取帮助信息\r\n");
			xprintf("help: 获取帮助信息\r\n");
			xprintf("list: 可用的函数列表\r\n\n");
			xprintf("id:   可用函数的ID列表\r\n\n");
			xprintf("hex:  参数16进制显示,后跟空格+数字即执行进制转换\r\n\n");
			xprintf("dec:  参数10进制显示,后跟空格+数字即执行进制转换\r\n\n");
			xprintf("请按照程序编写格式输入函数名及参数并以回车键结束.\r\n");
			xprintf("--------------------------ALIENTEK------------------------- \r\n");
#else
			xprintf("指令失效\r\n");
#endif
...
void usmart_exe(void)
{
	...
	for(i=0;i<pnum;i++)//输出参数
	{
		...
		{
			xprintf("%c",'"');
			xprintf("%s",usmart_dev.parm+usmart_get_parmpos(i));
			xprintf("%c",'"');
        ...
        else						  //参数是数字
		{
			...
			if(usmart_dev.sptype==SP_TYPE_DEC)
                xprintf("%lu",temp[i]);//10进制参数显示
			else
                xprintf("0X%X",temp[i]);//16进制参数显示
		}
		if(i!=pnum-1)
            xprintf(",");
	}
...
{
	if(TIM6->SR & TIM_FLAG_Update)//溢出中断
	{
		usmart_dev.scan();//执行usmart扫描
	}
	TIM6->SR &= ~TIM_FLAG_Update;//清除中断标志位
}
...
{
#if USMART_ENTIMX_SCAN==1
	Tim_Init(10, (u32)sysclk*100-1);//分频,时钟为10K ,100ms中断一次
#endif
